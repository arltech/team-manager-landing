...
/**
 * Port of v2-whatsapp.jsx → "Provas reais no WhatsApp" section.
 * Anonymized client feedback as WhatsApp-style chat bubbles: no names,
 * photos or phone numbers, ever.
 */
type Thread = { role: string; time: string; msgs: string[] };
...
function ThreadCard({ t, i }: { t: Thread; i: number }) {
  return (
    <div
      style={{ boxShadow: "0 24px 60px -18px rgba(0,0,0,.5)" }}
      className="rounded-[22px] overflow-hidden border border-white/10 bg-[#0b141a] flex flex-col"
    >
      {/* header */}
      <div className="flex items-center gap-3 px-[18px] py-3.5 bg-[#1f2c33] border-b border-black/30">
        <span
          style={{ background: AVA[i % AVA.length] }}
          className="w-10 h-10 rounded-full shrink-0 inline-flex items-center justify-center"
        >
          <Person />
        </span>
        ...
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8696a0" strokeWidth="2" className="ml-auto shrink-0">
          ...
          {t.msgs.map((m, j) => {
            ...
            return (
              <div
                ...
                className="max-w-[92%] bg-[#202c33] rounded-xl px-3 pt-[9px] pb-[7px] text-[#e9edef] text-[14.5px] leading-relaxed"
              >
                {m}
                {last && (
...
export function WhatsProof() {
  return (
    ...
    >
      <div
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 0%, rgba(37,211,102,.10), transparent 60%)" }}
        className="absolute inset-0"
      />
      <div className="relative max-w-[1100px] mx-auto px-7">
        <Reveal className="text-center mb-4">
          <span className="inline-flex items-center gap-2.5 text-xs font-bold tracking-[0.14em] uppercase text-[#25d366]">
        ...
        <Reveal className="text-center">
          <p className="text-white/60 text-[17px] max-w-[560px] mx-auto mb-[54px] leading-relaxed">
            Mensagens reais de quem usa o Team Manager no dia a dia. Nomes, fotos e telefones omitidos por privacidade.
          </p>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[22px] items-start">
          {THREADS.map((t, i) => (
            <Reveal key={i} delay={(i % 2) * 0.08} className="flex">
              <ThreadCard t={t} i={i} />
            </Reveal>
          ))}
        </div>
